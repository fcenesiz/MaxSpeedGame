...
public class BallJoystick implements Curd {
    ...
    public float maxSpeed = 500f; // m/s
    public float minSpeed = 100f; // m/s
    ...
    public void lateUpdate(float dt) {
        ...
        this.velocity.mulAdd(tmpVec, dt); // m/s
    ...
    private void joystick(){
        ...
        if (Gdx.input.isKeyPressed(Input.Keys.A)){
            tmpVec.add(-25f, 0); // m/s²
        }
        if (Gdx.input.isKeyPressed(Input.Keys.D)){
            tmpVec.add(25f, 0); // m/s²
        }
        if (tmpVec.len() > 0f){
            tmpVec.setLength(tmpVec.len() * 0.90f);
        }
